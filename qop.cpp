...
//将输入数据从ap_axiu转换为v_arr类型
static v_arr convV(ap_axiu<256,0,0,0> W) {
	#pragma HLS inline
	v_arr Wo;
...
//从输入流中读取数据到数组中
template <int lalala> void rmem(int iter, const v_arr *inW, v_arr W[Mopers/VDATA_SIZE])
{
	v_arr iW;
...
//索引到输入和权重的位置，运算后写入输出流
template <int lalala> void onemath(
	int iter, 
	int v, 
	v_arr inV[Mopers/VDATA_SIZE][Veclen][(Tsize/Mopers)/cores][Qop_paths], 
	v_arr W[Mopers/VDATA_SIZE][Qop_paths], 
